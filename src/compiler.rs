    ...
    fn compile(&self, pslab: &ParseSlab, cslab: &mut CompileSlab) -> Instruction {
            ...
            || lowest_op == EGTE
        {
            ...
            let mut out = match xss.first() {
                ...
                None => IConst(f64::NAN), // unreachable
            };
        ...
        match lowest_op {
            ...
            //          }
            ENE | EEQ | EGTE | ELTE | EGT | ELT => IConst(f64::NAN), // unreachable
        }
    }
}
    ...
    fn compile(&self, pslab: &ParseSlab, cslab: &mut CompileSlab) -> Instruction {
        match self {
            ...
            EFuncMin {
                ...
                for i in is {
                    rest.push(get_expr!(pslab, i).compile(pslab, cslab));
                }
                let mut out = IConst(0.0);
                let mut out_set = false;

...
impl Evaler for StdFunc {
    ...
    fn eval(&self, slab: &Slab, ns: &mut impl EvalNamespace) -> Result<f64, Error> {
        match self {
            ...
            EVar(name) => eval_var!(ns, name, Vec::new(), unsafe { slab.ps.char_buf_mut() }),
            EFunc { name, args: xis } => {
                let mut args = Vec::with_capacity(xis.len());
                for xi in xis {
                    args.push(get_expr!(slab.ps, xi).eval(slab, ns)?)
                }
                eval_var!(ns, name, args, unsafe { slab.ps.char_buf_mut() })
            }
...
impl Evaler for Instruction {
    ...
    fn eval(&self, slab: &Slab, ns: &mut impl EvalNamespace) -> Result<f64, Error> {
        match self {
            ...
            IVar(name) => eval_var!(ns, name, Vec::new(), unsafe { slab.ps.char_buf_mut() }),
            IFunc { name, args: ics } => {
